...
interface SubmissionRow {
  ...
  submitted_by: { first_name: string; last_name: string | null } | null
}
...
function personName(e: SubmissionRow['submitted_by']): string {
  if (!e) return '-'
...
async function loadSummary() {
  ...
  try {
    const response = await apiClient.get('/api/grooming-store/monitoring-summary', { params: { date: summaryDate.value } })
...
async function loadRows(page = 1) {
  ...
  errorMessage.value = ''
  try {
    const response = await apiClient.get('/api/grooming-store', {
      params: {
        page,
...
async function openDetail(row: SubmissionRow) {
  ...
  detail.value = null
  try {
    const response = await apiClient.get(`/api/grooming-store/${row.id}`)
...
<template>
  <div class="space-y-4">
    <div>
      <h1 class="text-lg font-semibold text-slate-800">Grooming Store Monitoring</h1>
      <p class="text-sm text-slate-400">Ringkasan kepatuhan kebersihan &amp; kerapian store.</p>
    </div>
    ...
    <div v-if="!summaryLoading && summary" class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
      <div class="rounded-2xl border border-slate-100 bg-white p-4">
        <p class="text-xl font-bold text-slate-900">{{ summary.total_branches }}</p>
        <p class="text-xs text-slate-400">Total Store</p>
      </div>
      <div class="rounded-2xl border border-slate-100 bg-white p-4">
        <p class="text-xl font-bold text-slate-900">{{ summary.assessed_count }}</p>
        <p class="text-xs text-slate-400">Assessed</p>
      </div>
      <div class="rounded-2xl border border-slate-100 bg-white p-4">
        <p class="text-xl font-bold text-amber-600">{{ summary.not_assessed_count }}</p>
      ...
      <table class="w-full text-left text-sm">
        <thead class="border-b border-slate-100 text-xs uppercase text-slate-400">
          <tr>
            ...
            <th class="px-4 py-3">Assessor</th>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.id" class="cursor-pointer border-b border-slate-50 hover:bg-slate-50" @click="openDetail(row)">
            <td class="px-4 py-3 font-medium text-slate-700">{{ row.branch?.name || '-' }}</td>
            <td class="px-4 py-3 text-slate-500">{{ personName(row.submitted_by) }}</td>
    ...
    <Teleport to="body">
      <BaseModal v-if="showDetailModal" title="Detail Grooming Store" @close="showDetailModal = false">
        ...
        <div v-else-if="detail" class="space-y-4">
          <div class="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><p class="text-xs text-slate-400">Assessor</p><p class="font-medium text-slate-700">{{ personName(detail.submitted_by) }}</p></div>
