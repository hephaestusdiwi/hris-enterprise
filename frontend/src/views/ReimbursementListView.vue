...
import { X, Loader2, AlertTriangle, Ban, Wallet, Paperclip, Filter } from 'lucide-vue-next'
import apiClient from '@/lib/axios'

type ReimbursementStatus = 'pending' | 'approved' | 'rejected' | 'cancelled'

interface Employee { id: number; first_name: string; last_name: string | null; department?: { name: string } | null }
interface Item { id: number; amount: string; notes: string | null; benefit: { name: string } }
interface Attachment { id: number; file_name: string; url: string | null }
interface StepDecision { id: number; sequence: number; status: string; notes: string | null; decided_at: string | null; approval_step: { name: string | null } }
interface ReimbursementRow {
  id: number
  employee: Employee
  policy: { id: number; name: string }
  balance: { id: number; assigned_amount: string | null }
...
async function loadReimbursements() {
  ...
  errorMessage.value = ''
  try {
    const response = await apiClient.get('/api/reimbursements', { params: statusFilter.value ? { status: statusFilter.value } : {} })
    reimbursements.value = response.data.data.data
  } catch {
    errorMessage.value = 'Gagal memuat daftar reimbursement.'
  } finally {
    loading.value = false
  }
}
...
const drawerTarget = ref<ReimbursementRow | null>(null)
const actionProcessing = ref(false)
const actionError = ref('')

async function openDrawer(row: ReimbursementRow) {
  actionError.value = ''
  showCancelForm.value = false
  showDisburseForm.value = false
  try {
    const response = await apiClient.get(`/api/reimbursements/${row.id}`)
    drawerTarget.value = response.data.data
    showDrawer.value = true
  } catch {
    errorMessage.value = 'Gagal memuat detail reimbursement.'
  }
}
...
async function submitCancel(row: ReimbursementRow) {
  if (!cancelReason.value.trim()) return
  actionProcessing.value = true
  actionError.value = ''
  try {
    await apiClient.post(`/api/reimbursements/${row.id}/cancel`, { reason: cancelReason.value })
    showCancelForm.value = false
    await Promise.all([loadReimbursements(), openDrawer(row)])
  } catch (err: any) {
    actionError.value = err.response?.data?.message || 'Gagal membatalkan request.'
  } finally {
...
async function submitDisburse(row: ReimbursementRow) {
  actionProcessing.value = true
  actionError.value = ''
  try {
    await apiClient.post(`/api/reimbursements/${row.id}/disburse`, { disbursement_note: disburseNote.value || null })
    showDisburseForm.value = false
    await Promise.all([loadReimbursements(), openDrawer(row)])
  } catch (err: any) {
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Reimbursement</h1>
        <p class="mt-1 text-sm text-slate-500">Semua pengajuan reimbursement karyawan.</p>
      </div>
      <div class="flex items-center gap-2">
        <Filter class="h-4 w-4 text-slate-400" :stroke-width="1.75" />
        <select v-model="statusFilter" @change="loadReimbursements" class="rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-primary focus:outline-none">
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          <tr v-if="reimbursements.length === 0">
            <td colspan="5" class="px-5 py-6 text-center text-sm text-slate-400">Tidak ada data.</td>
          </tr>
          <tr v-for="row in reimbursements" :key="row.id" @click="openDrawer(row)" class="cursor-pointer border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
            ...
            <td class="px-5 py-3.5 text-slate-500">{{ formatDate(row.transaction_date) }}</td>
            <td class="px-5 py-3.5 text-right font-medium text-slate-700">{{ formatCurrency(row.total_amount) }}</td>
            <td class="px-5 py-3.5">
              <span class="rounded-full px-2 py-0.5 text-xs font-medium" :class="statusBadgeClass[row.status]">{{ statusLabels[row.status] }}</span>
              <span v-if="row.disbursed_at" class="ml-1.5 text-xs text-emerald-600">· Dibayar</span>
            </td>
          </tr>
        </tbody>
      </table>
        ...
        <div class="h-full w-full max-w-lg overflow-y-auto bg-white shadow-xl">
          <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <div>
              ...
              <p class="text-sm text-slate-500">{{ drawerTarget.policy.name }}</p>
            </div>
          ...
          <div class="space-y-4 px-6 py-5">
            <span class="inline-block rounded-full px-2 py-0.5 text-xs font-medium" :class="statusBadgeClass[drawerTarget.status]">
            ...
            <div class="rounded-xl bg-primary-soft p-4">
              ...
              <p class="mt-1 text-xs text-slate-500">Transaksi {{ formatDate(drawerTarget.transaction_date) }}</p>
            </div>

            <div>
              <p class="mb-1.5 text-xs font-medium text-slate-500">Item</p>
              <ul class="space-y-1.5">
                <li v-for="item in drawerTarget.items" :key="item.id" class="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm">
                  <span class="text-slate-700">{{ item.benefit.name }}</span>
            ...
            <p v-if="drawerTarget.notes" class="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">Catatan: {{ drawerTarget.notes }}</p>

            <div v-if="drawerTarget.approval_request" class="space-y-1.5">
              <p class="text-xs font-medium text-slate-500">Riwayat Approval</p>
              <div v-for="step in drawerTarget.approval_request.step_decisions" :key="step.id" class="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-xs">
            ...
            <!-- Cancel form -->
            <div v-if="showCancelForm" class="space-y-2 rounded-xl border border-red-200 p-3">
              <textarea v-model="cancelReason" rows="2" placeholder="Alasan pembatalan..." class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-primary focus:outline-none"></textarea>
              <div class="flex gap-2">
                <button @click="showCancelForm = false" class="flex-1 rounded-lg border border-slate-200 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50">Batal</button>
                <button @click="submitCancel(drawerTarget)" :disabled="actionProcessing" class="flex-1 rounded-lg bg-red-600 py-2 text-xs font-medium text-white hover:bg-red-700 disabled:opacity-50">Ya, Batalkan</button>
              </div>
            </div>
          ...
          <div v-if="!showCancelForm && !showDisburseForm" class="space-y-2 border-t border-slate-100 px-6 py-4">
            <button
              v-if="drawerTarget.status === 'approved' && !drawerTarget.disbursed_at"
            ...
            >
              <Wallet class="h-4 w-4" :stroke-width="1.75" /> Catat Disbursement
            </button>
            <button
              v-if="['pending', 'approved'].includes(drawerTarget.status) && !drawerTarget.disbursed_at"
              @click="showCancelForm = true"
              class="flex w-full items-center justify-center gap-2 rounded-xl border border-red-200 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50"
            >
              <Ban class="h-4 w-4" :stroke-width="1.75" /> Batalkan Request
            </button>
          </div>
        </div>
      </div>
