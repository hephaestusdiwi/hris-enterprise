...
interface ApprovalApproverEmployee {
  id: number
  first_name: string
...
interface ApprovalStep {
  ...
  is_active: boolean
  approverEmployee?: ApprovalApproverEmployee | null
}

interface ApprovalRequest {
  id: number
  employee_movement_id: number
  employee_id: number
  approval_flow_id: number
  status: string
...
interface PendingApprovalDecision {
  id: number
  employee_movement_approval_request_id: number
...
const pendingApprovals = ref<PendingApprovalDecision[]>([])
...
async function load() {
  ...
  try {
    ...
    errorMessage.value =
      ...
      'Gagal memuat riwayat Employee Movement.'
  } finally {
    loading.value = false
  }
}
...
async function loadPendingApprovals() {
  ...
  try {
    const response = await apiClient.get(
      '/api/employee-movements/approvals/pending',
    )

    pendingApprovals.value = response.data.data ?? []
  } catch {
    // Jangan merusak halaman Employee Movement kalau endpoint approval
    // gagal / user tidak punya akses. Riwayat tetap ditampilkan.
...
function approvalFor(movementId: number): PendingApprovalDecision | null {
  return (
    pendingApprovals.value.find(
      (decision) =>
        decision.request?.employee_movement_id === movementId &&
        decision.status === 'pending',
    ) ?? null
  )
}

function approvalStepName(decision: PendingApprovalDecision | null): string {
  return decision?.approvalStep?.name ?? 'Approval'
}

function approvalApproverName(
  decision: PendingApprovalDecision | null,
): string {
  const employee = decision?.approvalStep?.approverEmployee
...
async function decideMovement(
  ...
  const movementId = decision.request?.employee_movement_id
  ...
  const confirmed = window.confirm(
    action === 'approve'
      ? 'Apakah Employee Movement ini ingin disetujui?'
      : 'Apakah Employee Movement ini ingin ditolak?',
  )
  ...
  try {
    await apiClient.post(
      `/api/employee-movements/approvals/${decision.id}/decide`,
      {
        action,
        notes: null,
      },
    ...
    if (detailTarget.value?.id === movementId) {
      detailTarget.value = null
    }
  } catch (error: any) {
    errorMessage.value =
      error?.response?.data?.message ??
      'Gagal memproses approval Employee Movement.'
  } finally {
...
const stats = computed(() => {
  ...
  const applied = movements.value.filter(
    (m) => m.status === 'applied',
  ).length

  const rejected = movements.value.filter(
    (m) => m.status === 'rejected',
  ).length
...
const myPendingCount = computed(() => pendingApprovals.value.length)
  ...
  <Transition name="page" appear>
    <div class="mx-auto max-w-6xl space-y-5 p-6">
      ...
      <div class="flex items-center gap-3">
        <span
          class="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary-soft text-primary-dark"
        >
        ...
        <div>
          ...
          <p class="mt-0.5 text-sm text-slate-500">
            Riwayat perubahan lifecycle employee (transfer, promosi, contract,
            status, dst).
          </p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-4">
        <div
          class="rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_1px_3px_rgba(15,23,42,0.04)] transition-shadow hover:shadow-[0_2px_8px_rgba(15,23,42,0.06)]"
        >
        ...
        >
          <div class="flex items-center justify-between">
            <p class="text-xs font-medium text-slate-400">My Approvals</p>

            <span
              class="flex h-8 w-8 items-center justify-center rounded-xl bg-sky-50 text-sky-600"
            >
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <span
              class="flex h-8 w-8 items-center justify-center rounded-xl bg-red-50 text-red-600"
            >
      ...
      <!-- Filter bar -->
      <div
        class="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-100 bg-white p-3 shadow-[0_1px_3px_rgba(15,23,42,0.04)]"
      >
        <div class="relative">
          <select
          ...
          >
            ...
            <option
              v-for="(label, value) in MOVEMENT_TYPE_LABELS"
              :key="value"
              :value="value"
            >
              {{ label }}
        ...
        >
          ...
          <input
            ...
            type="date"
            class="text-sm text-slate-600 outline-none"
          />
      ...
      >
        <div
          class="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-slate-100"
        >
        ...
        <p class="mt-1 max-w-sm text-xs text-slate-400">
          Belum ada Employee Movement yang tercatat. Riwayat transfer,
          promosi, atau perubahan status akan muncul di sini.
        </p>
      </div>
          ...
          <table class="w-full min-w-[1050px] text-left text-sm">
            ...
            >
              <tr>
                ...
                <th class="px-3 py-3 font-medium">Approval</th>
              ...
              >
                ...
                <td class="px-3 py-3.5">
                  <span
                    ...
                    :class="typeColor(m.movement_type)"
                  >
                    <component
                      :is="typeIcon(m.movement_type)"
                      class="h-3 w-3"
                    />
                ...
                <td class="px-3 py-3.5">
                  <div
                    v-if="approvalFor(m.id)"
                    class="min-w-[210px]"
                  >
                    <div class="flex items-center gap-1.5">
                      <span
                        class="inline-flex items-center rounded-full bg-amber-50 px-2 py-1 text-[11px] font-semibold text-amber-700"
                      >
                        {{ approvalStepName(approvalFor(m.id)) }}
                      </span>

                      <span
                        class="text-[11px] font-medium text-slate-500"
                      >
                        {{ approvalApproverName(approvalFor(m.id)) }}
                      </span>
                    </div>
                    ...
                    <div class="mt-2 flex items-center gap-1.5">
                      <button
                        type="button"
                        :disabled="
                          decidingId === approvalFor(m.id)?.id
                        "
                        @click="
                          approvalFor(m.id) &&
                          decideMovement(
                        ...
                        "
                        class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-500 px-2.5 py-1.5 text-[11px] font-semibold text-white transition-colors hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        "
                        class="inline-flex items-center gap-1.5 rounded-lg bg-red-500 px-2.5 py-1.5 text-[11px] font-semibold text-white transition-colors hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                  ...
                  <div
                    v-else-if="m.status === 'pending_approval'"
      ...
      <Transition name="overlay">
        ...
        >
          <Transition name="panel" appear>
            <div
              v-if="detailTarget"
              class="max-h-[85vh] w-full max-w-lg overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-slate-100"
            >
              <!-- Modal Header -->
              <div
              ...
              >
                <div class="flex items-center gap-3">
                  <span
                    ...
                    :class="typeColor(detailTarget.movement_type)"
                  >
                    <component
                      :is="typeIcon(detailTarget.movement_type)"
                      class="h-5 w-5"
                    />
                  </span>
                    ...
                    <h2 class="text-sm font-semibold text-slate-800">
                      {{
                        MOVEMENT_TYPE_LABELS[
                          detailTarget.movement_type
                        ] ?? detailTarget.movement_type
                      }}
                    </h2>

                    <p class="text-xs text-slate-400">
                      {{ detailTarget.employee.first_name }}
                      {{ detailTarget.employee.last_name }}
                    </p>
                  </div>
                </div>
              ...
              <!-- Modal Body -->
              <div
                class="max-h-[calc(85vh-140px)] space-y-5 overflow-y-auto px-6 py-5"
              >
                <!-- Meta -->
                <div class="flex flex-wrap items-center gap-2">
                  <span
                    class="inline-flex items-center gap-1.5 rounded-full bg-slate-50 px-2.5 py-1 text-xs text-slate-600"
                  >
                    <Calendar class="h-3 w-3 text-slate-400" />
                    {{ detailTarget.effective_date }}
                  </span>

                  <span
                    class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium"
                    :class="
                      STATUS_STYLE[detailTarget.status] ??
                      'bg-slate-100 text-slate-500'
                    "
                  >
                    <component
                      :is="statusIcon(detailTarget.status)"
                      class="h-3 w-3"
                    />
                  ...
                  <span
                    v-if="detailTarget.requested_by"
                    class="inline-flex items-center gap-1.5 rounded-full bg-slate-50 px-2.5 py-1 text-xs text-slate-600"
                  >
                    <User class="h-3 w-3 text-slate-400" />
                    {{ detailTarget.requested_by.name }}
                  </span>

                  <span
                    v-if="detailTarget.applied_at"
                    class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-xs text-emerald-700"
                  >
                    <CheckCircle2 class="h-3 w-3" />
                    Applied {{ detailTarget.applied_at }}
                  </span>
                </div>

                <!-- Approval section -->
                <div
                  v-if="approvalFor(detailTarget.id)"
                ...
                >
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      ...
                      >
                        {{ approvalStepName(approvalFor(detailTarget.id)) }}
                      </p>
                    ...
                    <span
                      class="inline-flex items-center gap-1.5 rounded-full bg-white px-2.5 py-1 text-[11px] font-medium text-amber-700 ring-1 ring-amber-100"
                    >
                      <Clock class="h-3 w-3" />
                      Pending
                    </span>
                  ...
                  <div class="mt-3 flex items-center gap-2">
                    <button
                      ...
                      :disabled="
                        decidingId ===
                        approvalFor(detailTarget.id)?.id
                      "
                      @click="
                        approvalFor(detailTarget.id) &&
                        decideMovement(
                      ...
                      "
                      class="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-500 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      <CheckCircle2 class="h-3.5 w-3.5" />
                      Approve
                    </button>
                    ...
                    <button
                      ...
                      "
                      class="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-red-500 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      <XCircle class="h-3.5 w-3.5" />
                      Reject
                    </button>
                ...
                <div>
                  <div
                    class="mb-2 flex items-center justify-between px-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400"
                  >
                    <span>Field</span>
                  ...
                  <div class="space-y-1">
                    <div
                      ...
                      :class="
                        isChanged(String(field), after)
                          ? 'bg-primary-soft/40'
                          : ''
                      "
                    >
                      <span
                        class="w-32 shrink-0 text-xs font-medium text-slate-500"
                      >
                        {{ FIELD_LABELS[field] ?? field }}
                      </span>

                      <div
                        class="flex flex-1 items-center justify-end gap-2 text-xs"
                      >
                        <template
                          v-if="isChanged(String(field), after)"
                        >
                          <span
                            class="text-slate-400 line-through decoration-slate-300"
                          >
                            {{
                              detailTarget.before_snapshot[field] ??
                              '-'
                            }}
                          </span>
