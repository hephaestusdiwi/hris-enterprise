...
// ---------- DATA ----------
const allowances = ref<AllowanceRow[]>([])
const employees = ref<Employee[]>([])
const departments = ref<Department[]>([])
const allowanceComponents = ref<SalaryComponent[]>([])
...
async function loadAllowances() {
  ...
  errorMessage.value = ''
  try {
    const response = await apiClient.get('/api/employee-allowances', { params: buildParams() })
    ...
    errorMessage.value = 'Gagal memuat daftar allowance.'
  } finally {
    loading.value = false
  }
}
...
async function loadSummary() {
  try {
    const response = await apiClient.get('/api/employee-allowances/summary', { params: buildParams() })
...
async function loadReferenceData() {
  ...
  allowanceComponents.value = componentRes.data.data.data.filter(
    (c: SalaryComponent) => c.category === 'allowance' && c.is_addition,
  )
}
...
const allSelected = computed(() => allowances.value.length > 0 && allowances.value.every((a) => selectedIds.value.has(a.id)))

function toggleSelectAll() {
  if (allSelected.value) allowances.value.forEach((a) => selectedIds.value.delete(a.id))
  else allowances.value.forEach((a) => selectedIds.value.add(a.id))
}
...
const componentsForEmployee = computed(() => {
  ...
  return emp ? allowanceComponents.value.filter((c) => c.company_id === emp.company_id) : []
})
...
async function submitCreate() {
  ...
  try {
    await apiClient.post('/api/employee-allowances', form)
    showCreateModal.value = false
    await Promise.all([loadAllowances(), loadSummary()])
  } catch (err: any) {
    createError.value = err.response?.data?.message || 'Gagal membuat allowance.'
  } finally {
    saving.value = false
  }
}
...
async function bulkMarkReady() {
  ...
  try {
    await apiClient.post('/api/employee-allowances/bulk-mark-ready', { ids: Array.from(selectedIds.value) })
...
async function submitBulkVoid() {
  ...
  bulkProcessing.value = true
  try {
    await apiClient.post('/api/employee-allowances/bulk-void', {
...
async function handleImportFile(event: Event) {
  ...
  try {
    ...
    const response = await apiClient.post('/api/employee-allowances/import', formData, {
...
function handleExport() {
  ...
  window.open(`${baseUrl}/api/employee-allowances/export?${params.toString()}`, '_blank')
}

// ---------- DRAWER DETAIL ----------
const showDrawer = ref(false)
const drawerTarget = ref<AllowanceRow | null>(null)
const drawerVoidReason = ref('')
const showDrawerVoidForm = ref(false)

function openDrawer(row: AllowanceRow) {
...
async function submitDrawerVoid() {
  ...
  bulkProcessing.value = true
  try {
    await apiClient.post(`/api/employee-allowances/${drawerTarget.value.id}/void`, { reason: drawerVoidReason.value })
    closeDrawer()
    await Promise.all([loadAllowances(), loadSummary()])
  } catch (err: any) {
    alert(err.response?.data?.message || 'Gagal void allowance.')
  } finally {
    bulkProcessing.value = false
  }
}
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Employee Allowance</h1>
        <p class="mt-1 text-sm text-slate-500">Additional Income sekali-pakai per periode payroll.</p>
      </div>
    ...
    <div v-if="importResult" class="rounded-xl border border-slate-100 bg-white p-4 text-sm">
      <p class="font-medium text-slate-700">{{ importResult.created }} allowance berhasil diimport.</p>
    ...
    <div class="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-100 bg-white p-4">
      ...
      <div>
        ...
        <select v-model="filters.salary_component_id" class="rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-primary focus:outline-none">
          ...
          <option v-for="c in allowanceComponents" :key="c.id" :value="c.id">{{ c.name }}</option>
        </select>
      </div>
    ...
    <div v-if="selectedIds.size > 0" class="flex items-center justify-between rounded-2xl bg-primary-soft px-5 py-3">
      <p class="text-sm font-medium text-primary-dark">{{ selectedIds.size }} allowance dipilih</p>
      <div class="flex items-center gap-2">
    ...
    <div v-else-if="allowances.length === 0" class="rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-400">
      Tidak ada allowance untuk filter ini.
    </div>
    ...
    <div v-if="meta.last_page > 1" class="flex items-center justify-between text-sm text-slate-500">
      <p>Total {{ meta.total }} allowance</p>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 px-4 py-8">
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-xl">
          <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <h2 class="text-lg font-semibold text-slate-900">Tambah Allowance</h2>
          ...
          <form @submit.prevent="submitCreate" class="space-y-4 px-6 py-5">
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-slate-700">Salary Component (Allowance)</label>
              ...
              <p v-if="componentsForEmployee.length === 0 && form.employee_id" class="mt-1 text-xs text-amber-600">
                Belum ada Salary Component kategori Allowance untuk company employee ini.
              </p>
            </div>
      ...
      <div v-if="showBulkVoidModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 px-4 py-8">
        <div class="w-full max-w-md rounded-2xl bg-white shadow-xl">
          <div class="border-b border-slate-100 px-6 py-4">
            <h2 class="text-lg font-semibold text-slate-900">Void {{ selectedIds.size }} Allowance</h2>
          </div>
    ...
    <Teleport to="body">
      <div v-if="showDrawer" class="fixed inset-0 z-50 flex justify-end bg-slate-900/30">
        <div class="h-full w-full max-w-md overflow-y-auto bg-white shadow-xl">
          <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <h2 class="text-lg font-semibold text-slate-900">Detail Allowance</h2>
          ...
          <div v-if="drawerTarget" class="space-y-5 px-6 py-5">
            ...
            <div class="space-y-3">
              ...
              <div class="rounded-xl bg-slate-50 p-4">
                ...
                <p class="text-xl font-semibold text-slate-800">{{ formatCurrency(drawerTarget.amount) }}</p>
              </div>
