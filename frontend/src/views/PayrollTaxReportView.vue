...
const detailRows = ref<TaxDetailRow[]>([])
...
async function loadReport() {
  ...
  try {
    if (viewMode.value === 'detail') {
      const response = await apiClient.get('/api/payroll-reports/tax/detail', { params: { ...filterParams(), page: filters.page } })
      ...
      const response = await apiClient.get('/api/payroll-reports/tax/summary', { params: filterParams() })
      summary.value = response.data.data
    }
  } catch (err: any) {
    errorMessage.value = err.response?.data?.message || 'Gagal memuat tax report.'
  } finally {
    loading.value = false
  }
}
...
function handleExport(format: 'excel' | 'pdf') {
  ...
  try {
    ...
    window.open(`${baseUrl}/api/payroll-reports/tax/${endpoint}/export/${format}?${params.toString()}`, '_blank')
  } finally {
...
<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Tax Reports</h1>
      <p class="mt-1 text-sm text-slate-500">PPh21 bulanan per karyawan atau ringkasan per periode.</p>
    </div>
    ...
    <div v-else-if="viewMode === 'summary' && summary" class="rounded-2xl border border-slate-100 bg-white p-6">
      ...
      <div class="space-y-3 text-sm">
        <div class="flex justify-between"><span class="text-slate-500">Gross Earning</span><span class="font-medium text-slate-800">{{ formatCurrency(summary.gross_earning) }}</span></div>
      </div>
      <div class="mt-5 flex justify-between border-t border-slate-100 pt-4">
        <span class="text-base font-semibold text-slate-800">Total PPh21</span>
        <span class="text-base font-semibold text-primary-dark">{{ formatCurrency(summary.tax_amount) }}</span>
      </div>
    </div>
        ...
        <table class="w-full text-left text-sm">
          <thead>
            <tr class="border-b border-slate-100 bg-slate-50/60">
              ...
              <th class="whitespace-nowrap px-4 py-3 font-medium text-slate-500">Keterangan</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in detailRows" :key="row.employee_id" class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
              ...
              <td class="px-4 py-3 text-right font-medium text-slate-800">{{ formatCurrency(row.tax_amount) }}</td>
              <td class="px-4 py-3">
                <span v-if="row.is_annual_reconciliation" class="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-600">Rekonsiliasi Tahunan</span>
                <span v-else class="text-xs text-slate-400">Bulanan</span>
              </td>
            </tr>
          </tbody>
        </table>
