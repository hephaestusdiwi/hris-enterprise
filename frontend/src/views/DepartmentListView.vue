...
async function loadDepartments() {
  ...
  try {
    ...
    errorMessage.value = 'Gagal memuat daftar department.'
  } finally {
    loading.value = false
  }
}
...
function openEditModal(department: DepartmentRow) {
  isEditing.value = true
  formError.value = ''
  form.id = department.id
  form.company_id = department.company.id
...
async function handleSubmit() {
  ...
  try {
    if (isEditing.value) {
      await apiClient.put(`/api/departments/${form.id}`, payload)
    } else {
...
async function handleDelete(department: DepartmentRow) {
  if (!confirm(`Hapus department "${department.name}"?`)) return
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Department</h1>
        <p class="mt-1 text-sm text-slate-500">Kelola unit organisasi di setiap perusahaan.</p>
      </div>
      <button
    ...
    <p v-if="companies.length === 0 && !loading" class="rounded-xl bg-amber-50 p-4 text-sm text-amber-700">
      Belum ada company. Tambahkan company terlebih dahulu sebelum membuat department.
    </p>
    ...
    <div v-else class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)]">
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          ...
          >
            <td class="px-5 py-3.5 font-medium text-slate-800">{{ department.name }}</td>
            <td class="px-5 py-3.5 text-slate-500">{{ department.code }}</td>
            <td class="px-5 py-3.5 text-slate-500">{{ department.company.name }}</td>
            <td class="px-5 py-3.5">
              <span
                class="rounded-full px-2.5 py-1 text-xs font-medium"
                :class="department.is_active ? 'bg-primary-soft text-primary-dark' : 'bg-slate-100 text-slate-500'"
              >
        ...
        <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
          <div class="mb-5 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-900">
              {{ isEditing ? 'Edit Department' : 'Tambah Department' }}
            </h2>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-slate-700">Nama Department</label>
              <input
                v-model="form.name"
                type="text"
                required
