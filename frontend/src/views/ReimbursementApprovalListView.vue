...
interface PendingDecision {
  ...
  request: {
    ...
    request: {
      ...
      notes: string | null
      items: { id: number; amount: string; benefit: { name: string } }[]
    }
  }
}
...
async function loadDecisions() {
  ...
  errorMessage.value = ''
  try {
    const response = await apiClient.get('/api/reimbursement-approvals')
...
async function submitDecision() {
  ...
  modalError.value = ''
  try {
    await apiClient.post(`/api/reimbursement-approvals/${target.value.id}/decide`, {
...
<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Reimbursement Approval</h1>
      <p class="mt-1 text-sm text-slate-500">Pengajuan reimbursement yang menunggu keputusan Anda.</p>
    </div>
    ...
    <div v-else-if="decisions.length === 0" class="rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-400">
      Tidak ada reimbursement yang menunggu persetujuan Anda.
    </div>
      ...
      <table class="w-full text-left text-sm">
        <thead>
          <tr class="border-b border-slate-100 bg-slate-50/60">
            ...
            <th class="px-5 py-3 font-medium text-slate-500">Item</th>
            ...
            <td class="px-5 py-3.5">
              <div class="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p class="text-xs text-slate-400">{{ decision.request.request.transaction_date }}</p>
                </div>
              </div>
            </td>
            <td class="px-5 py-3.5 text-slate-500">
              {{ decision.request.request.items.map(i => i.benefit.name).join(', ') }}
            </td>
            <td class="px-5 py-3.5 text-right font-medium text-slate-700">{{ formatCurrency(decision.request.request.total_amount) }}</td>
            <td class="px-5 py-3.5">
        ...
        <div class="w-full max-w-md rounded-2xl bg-white shadow-xl">
          <div class="border-b border-slate-100 px-6 py-4">
            <h2 class="text-lg font-semibold text-slate-900">
              {{ modalMode === 'approve' ? 'Setujui' : 'Tolak' }} Reimbursement
            </h2>
          ...
          <div class="space-y-4 px-6 py-5">
            <div class="rounded-xl bg-slate-50 p-3 text-sm text-slate-600">
              {{ formatCurrency(target.request.request.total_amount) }} ·
              {{ target.request.request.items.map(i => i.benefit.name).join(', ') }}
              <p v-if="target.request.request.notes" class="mt-1 text-xs text-slate-400">{{ target.request.request.notes }}</p>
            </div>
