...
async function loadReport() {
  ...
  errorMessage.value = ''
  try {
    const response = await apiClient.get('/api/payroll-reports/non-regular/detail', { params: filterParams() })
    rows.value = response.data.data
  } catch (err: any) {
    errorMessage.value = err.response?.data?.message || 'Gagal memuat Non-Regular Report. Pastikan Anda punya izin.'
  } finally {
    loading.value = false
  }
}
...
function handleExport(format: 'excel' | 'pdf') {
  ...
  try {
    ...
    window.open(`${baseUrl}/api/payroll-reports/non-regular/detail/export/${format}?${params.toString()}`, '_blank')
  } finally {
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Non-Regular Payroll Report</h1>
        <p class="mt-1 text-sm text-slate-500">Detail Bonus/Incentive/Commission/One-Time Earning/Deduction per employee.</p>
      </div>
    ...
    <div class="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-100 bg-white p-4">
      ...
      <div>
        ...
        <select v-model="filters.status" @change="loadReport" class="rounded-lg border border-slate-200 px-3 py-1.5 text-sm">
    ...
    <div v-else-if="rows.length === 0" class="rounded-xl bg-slate-50 p-4 text-sm text-slate-400">
      Tidak ada data payroll non-reguler untuk filter ini.
    </div>
      ...
      <table class="w-full text-left text-sm">
        <thead>
          <tr class="border-b border-slate-100 bg-slate-50/60">
            ...
            <th class="px-5 py-3 font-medium text-slate-500">Taxable</th>
            <th class="px-5 py-3 text-right font-medium text-slate-500">Net Payslip</th>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.id" class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
            ...
            <td class="px-5 py-3.5 text-slate-700">{{ row.component_name }}</td>
            <td class="px-5 py-3.5 text-slate-500 capitalize">{{ row.category.replace('_', ' ') }}</td>
            <td class="px-5 py-3.5 text-right font-medium" :class="row.is_addition ? 'text-primary-dark' : 'text-red-600'">
              {{ row.is_addition ? '+' : '-' }}{{ formatCurrency(row.amount) }}
            </td>
            <td class="px-5 py-3.5 text-slate-500">{{ row.is_taxable ? 'Ya' : 'Tidak' }}</td>
            <td class="px-5 py-3.5 text-right text-slate-500">{{ formatCurrency(row.net_pay) }}</td>
            <td class="px-5 py-3.5">
