<script setup lang="ts">
import { ref, onMounted, reactive, computed } from 'vue'
import { Plus, Pencil, Trash2, X, RefreshCw, Copy, Check, AlertTriangle, MonitorSmartphone } from 'lucide-vue-next'
import { useRouter } from 'vue-router'
import apiClient from '@/lib/axios'

interface Company {
  id: number
  name: string
}

interface Branch {
  id: number
  name: string
  company_id: number
}

type DeviceType = 'kiosk' | 'face_recognition' | 'fingerprint' | 'qr_scanner' | 'other'

interface AttendanceDeviceRow {
  id: number
  company_id: number
  branch_id: number | null
  name: string
  type: DeviceType
  last_used_at: string | null
  is_active: boolean
  company: Company
  branch: Branch | null
}

const typeLabels: Record<DeviceType, string> = {
  kiosk: 'Tablet Kiosk',
  face_recognition: 'Face Recognition Terminal',
  fingerprint: 'Fingerprint Device',
  qr_scanner: 'QR Scanner',
  other: 'Lainnya',
}

const devices = ref<AttendanceDeviceRow[]>([])
const companies = ref<Company[]>([])
const branches = ref<Branch[]>([])
const loading = ref(true)
const errorMessage = ref('')
const router = useRouter()

const showModal = ref(false)
const isEditing = ref(false)
const saving = ref(false)
const formError = ref('')

const form = reactive({
  id: 0,
  company_id: 0,
  branch_id: null as number | null,
  name: '',
  type: 'kiosk' as DeviceType,
  is_active: true,
})

const filteredBranches = computed(() => branches.value.filter((b) => b.company_id === form.company_id))

// ---------- TOKEN MODAL ----------
const showTokenModal = ref(false)
const tokenModalTitle = ref('')
const generatedToken = ref('')
const tokenCopied = ref(false)

function openTokenModal(title: string, token: string) {
  tokenModalTitle.value = title
  generatedToken.value = token
  tokenCopied.value = false
  showTokenModal.value = true
}

async function copyToken() {
  try {
    await navigator.clipboard.writeText(generatedToken.value)
    tokenCopied.value = true
    setTimeout(() => (tokenCopied.value = false), 2000)
  } catch {
    alert('Gagal menyalin token, silakan copy manual.')
  }
}

function closeTokenModal() {
  showTokenModal.value = false
  generatedToken.value = ''
}

async function loadDevices() {
  loading.value = true
  errorMessage.value = ''
  try {
    const response = await apiClient.get('/api/attendance-devices')
    devices.value = response.data.data.data
  } catch {
    errorMessage.value = 'Gagal memuat daftar attendance device.'
  } finally {
    loading.value = false
  }
}

async function loadReferenceData() {
  const [companyRes, branchRes] = await Promise.all([
    apiClient.get('/api/companies'),
    apiClient.get('/api/branches'),
  ])
  companies.value = companyRes.data.data.data
  branches.value = branchRes.data.data.data
}

function resetForm() {
  form.id = 0
  form.company_id = companies.value[0]?.id ?? 0
  form.branch_id = null
  form.name = ''
  form.type = 'kiosk'
  form.is_active = true
}

function openCreateModal() {
  isEditing.value = false
  formError.value = ''
  resetForm()
  showModal.value = true
}

function openEditModal(row: AttendanceDeviceRow) {
  isEditing.value = true
  formError.value = ''
  form.id = row.id
  form.company_id = row.company_id
  form.branch_id = row.branch_id
  form.name = row.name
  form.type = row.type
  form.is_active = row.is_active
  showModal.value = true
}

function closeModal() {
  showModal.value = false
}

async function handleSubmit() {
  formError.value = ''
  saving.value = true

  const payload = {
    company_id: form.company_id,
    branch_id: form.branch_id,
    name: form.name,
    type: form.type,
    is_active: form.is_active,
  }

  try {
    if (isEditing.value) {
      await apiClient.put(`/api/attendance-devices/${form.id}`, payload)
      showModal.value = false
      await loadDevices()
    } else {
      const response = await apiClient.post('/api/attendance-devices', payload)
      showModal.value = false
      await loadDevices()
      openTokenModal(`Device "${form.name}" berhasil dibuat`, response.data.data.token)
    }
  } catch (err: any) {
    formError.value = err.response?.data?.message || 'Terjadi kesalahan, silakan coba lagi.'
  } finally {
    saving.value = false
  }
}

async function handleRegenerateToken(row: AttendanceDeviceRow) {
  if (!confirm(`Generate ulang token untuk device "${row.name}"? Token lama langsung tidak berlaku.`)) return

  try {
    const response = await apiClient.post(`/api/attendance-devices/${row.id}/regenerate-token`)
    openTokenModal(`Token baru untuk "${row.name}"`, response.data.data.token)
  } catch {
    alert('Gagal generate ulang token.')
  }
}

async function handleDelete(row: AttendanceDeviceRow) {
  if (!confirm(`Hapus attendance device "${row.name}"?`)) return

  try {
    await apiClient.delete(`/api/attendance-devices/${row.id}`)
    await loadDevices()
  } catch {
    alert('Gagal menghapus attendance device.')
  }
}

function formatLastUsed(value: string | null): string {
  if (!value) return 'Belum pernah dipakai'
  return new Date(value.replace(' ', 'T')).toLocaleString('id-ID', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
    hour: '2-digit',
    minute: '2-digit',
  })
}

onMounted(() => {
  loadDevices()
  loadReferenceData()
})
</script>

<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Attendance Device</h1>
        <p class="mt-1 text-sm text-slate-500">Kelola perangkat absensi (kiosk, face recognition, fingerprint, QR scanner).</p>
      </div>
      <button
        @click="openCreateModal"
        :disabled="companies.length === 0"
        class="flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-dark disabled:opacity-50"
      >
        <Plus class="h-4 w-4" :stroke-width="2" />
        Tambah Device
      </button>
    </div>

    <p v-if="companies.length === 0 && !loading" class="rounded-xl bg-amber-50 p-4 text-sm text-amber-700">
      Belum ada company. Tambahkan company terlebih dahulu sebelum membuat attendance device.
    </p>

    <div v-if="loading" class="text-sm text-slate-400">Memuat data...</div>
    <div v-else-if="errorMessage" class="rounded-xl bg-red-50 p-4 text-sm text-red-600">
      {{ errorMessage }}
    </div>

    <div v-else class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)]">
      <table class="w-full text-left text-sm">
        <thead>
          <tr class="border-b border-slate-100 bg-slate-50/60">
            <th class="px-5 py-3 font-medium text-slate-500">Nama</th>
            <th class="px-5 py-3 font-medium text-slate-500">Tipe</th>
            <th class="px-5 py-3 font-medium text-slate-500">Company / Branch</th>
            <th class="px-5 py-3 font-medium text-slate-500">Terakhir Dipakai</th>
            <th class="px-5 py-3 font-medium text-slate-500">Status</th>
            <th class="px-5 py-3 text-right font-medium text-slate-500">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr
            v-for="row in devices"
            :key="row.id"
            class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50"
          >
            <td class="px-5 py-3.5 font-medium text-slate-800">{{ row.name }}</td>
            <td class="px-5 py-3.5 text-slate-500">{{ typeLabels[row.type] }}</td>
            <td class="px-5 py-3.5 text-slate-500">
              {{ row.company.name }}
              <span v-if="row.branch"> / {{ row.branch.name }}</span>
            </td>
            <td class="px-5 py-3.5 text-slate-500">{{ formatLastUsed(row.last_used_at) }}</td>
            <td class="px-5 py-3.5">
              <span
                class="rounded-full px-2.5 py-1 text-xs font-medium"
                :class="row.is_active ? 'bg-primary-soft text-primary-dark' : 'bg-slate-50 text-slate-400'"
              >
                {{ row.is_active ? 'Aktif' : 'Nonaktif' }}
              </span>
            </td>
            <td class="px-5 py-3.5">
              <div class="flex items-center justify-end gap-1">
                <button
                  @click="handleRegenerateToken(row)"
                  class="rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
                  title="Generate ulang token"
                >
                  <RefreshCw class="h-4 w-4" :stroke-width="1.75" />
                </button>
                <button
                  @click="router.push(`/attendance-devices/${row.id}/office-qr`)"
                  class="rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
                  title="Tampilkan Office QR"
                >
                  <MonitorSmartphone class="h-4 w-4" :stroke-width="1.75" />
                </button>
                <button
                  @click="openEditModal(row)"
                  class="rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
                >
                  <Pencil class="h-4 w-4" :stroke-width="1.75" />
                </button>
                <button
                  @click="handleDelete(row)"
                  class="rounded-lg p-2 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-500"
                >
                  <Trash2 class="h-4 w-4" :stroke-width="1.75" />
                </button>
              </div>
            </td>
          </tr>
        </tbody>
      </table>
    </div>

    <!-- CREATE/EDIT MODAL -->
    <Teleport to="body">
      <div
        v-if="showModal"
        class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 px-4 py-8"
      >
        <div class="flex max-h-full w-full max-w-lg flex-col rounded-2xl bg-white shadow-xl">
          <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <h2 class="text-lg font-semibold text-slate-900">
              {{ isEditing ? 'Edit Attendance Device' : 'Tambah Attendance Device' }}
            </h2>
            <button @click="closeModal" class="rounded-lg p-1 text-slate-400 hover:bg-slate-50">
              <X class="h-5 w-5" />
            </button>
          </div>

          <form @submit.prevent="handleSubmit" class="flex-1 space-y-4 overflow-y-auto px-6 py-5">
            <div>
              <label class="mb-1 block text-sm font-medium text-slate-700">Nama Device</label>
              <input
                v-model="form.name"
                type="text"
                required
                placeholder="Kiosk Lobby Lantai 1"
                class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-primary focus:outline-none"
              />
            </div>

            <div>
              <label class="mb-1 block text-sm font-medium text-slate-700">Tipe Device</label>
              <select
                v-model="form.type"
                required
                class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-primary focus:outline-none"
              >
                <option v-for="(label, value) in typeLabels" :key="value" :value="value">{{ label }}</option>
              </select>
            </div>

            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Company</label>
                <select
                  v-model.number="form.company_id"
                  required
                  @change="form.branch_id = null"
                  class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-primary focus:outline-none"
                >
                  <option v-for="c in companies" :key="c.id" :value="c.id">{{ c.name }}</option>
                </select>
              </div>
              <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Branch</label>
                <select
                  v-model="form.branch_id"
                  class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-primary focus:outline-none"
                >
                  <option :value="null">Semua Branch</option>
                  <option v-for="b in filteredBranches" :key="b.id" :value="b.id">{{ b.name }}</option>
                </select>
              </div>
            </div>
            <p class="text-xs text-slate-400">
              Kosongkan Branch kalau device ini boleh dipakai employee dari branch mana aja dalam company tersebut.
            </p>

            <label class="flex cursor-pointer items-center justify-between rounded-xl border border-slate-200 px-4 py-3">
              <p class="text-sm font-medium text-slate-700">Aktif</p>
              <input v-model="form.is_active" type="checkbox" class="peer sr-only" />
              <div class="relative h-6 w-11 shrink-0 rounded-full bg-slate-200 transition-colors after:absolute after:left-0.5 after:top-0.5 after:h-5 after:w-5 after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:bg-primary peer-checked:after:translate-x-5"></div>
            </label>

            <p v-if="formError" class="text-sm text-red-600">{{ formError }}</p>
          </form>

          <div class="border-t border-slate-100 px-6 py-4">
            <button
              @click="handleSubmit"
              :disabled="saving"
              class="w-full rounded-xl bg-primary py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-dark disabled:opacity-50"
            >
              {{ saving ? 'Menyimpan...' : 'Simpan' }}
            </button>
          </div>
        </div>
      </div>
    </Teleport>

    <!-- TOKEN MODAL -->
    <Teleport to="body">
      <div
        v-if="showTokenModal"
        class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 px-4 py-8"
      >
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-xl">
          <div class="border-b border-slate-100 px-6 py-4">
            <h2 class="text-lg font-semibold text-slate-900">{{ tokenModalTitle }}</h2>
          </div>

          <div class="space-y-4 px-6 py-5">
            <div class="flex items-start gap-2 rounded-xl bg-amber-50 p-3 text-xs text-amber-700">
              <AlertTriangle class="mt-0.5 h-4 w-4 shrink-0" :stroke-width="1.75" />
              <p>Simpan token ini sekarang juga. Setelah ditutup, token ini tidak akan ditampilkan lagi — kalau hilang, harus generate ulang.</p>
            </div>

            <div class="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
              <code class="flex-1 break-all font-mono text-xs text-slate-700">{{ generatedToken }}</code>
              <button
                @click="copyToken"
                class="flex shrink-0 items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-100"
              >
                <Check v-if="tokenCopied" class="h-3.5 w-3.5 text-primary-dark" :stroke-width="2" />
                <Copy v-else class="h-3.5 w-3.5" :stroke-width="1.75" />
                {{ tokenCopied ? 'Tersalin' : 'Copy' }}
              </button>
            </div>

            <p class="text-xs text-slate-400">
              Pasang token ini di header <code class="rounded bg-slate-100 px-1 py-0.5">X-Device-Token</code> pada request dari device.
            </p>
          </div>

          <div class="border-t border-slate-100 px-6 py-4">
            <button
              @click="closeTokenModal"
              class="w-full rounded-xl bg-primary py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-dark"
            >
              Sudah Disimpan, Tutup
            </button>
          </div>
        </div>
      </div>
    </Teleport>
  </div>
</template>