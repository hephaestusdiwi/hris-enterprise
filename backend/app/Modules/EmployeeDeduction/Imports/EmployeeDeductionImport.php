...
namespace App\Modules\EmployeeDeduction\Imports;
...
class EmployeeDeductionImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            if ($component->company_id !== $employee->company_id || $component->category->value !== 'deduction' || $component->is_addition) {
                $this->errors[] = "Baris {$rowNumber}: salary_component_code '{$row['salary_component_code']}' bukan Deduction yang valid untuk employee ini.";
            ...
            $deduction = EmployeeDeduction::create([
                ...
                'status' => EmployeeDeductionStatus::Draft->value,
                'created_by_user_id' => $this->actorUserId,
            ]);

            $this->created[] = $deduction->id;
        }
    }
}