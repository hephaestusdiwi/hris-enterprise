...
class NewJoinerController extends Controller
{
    public function __construct(
        ...
        private NewJoinerConversionService $conversionService,
    ) {}
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar New Joiner berhasil diambil.',
            'data' => $newJoiners,
        ]);
    }
    ...
    public function show(NewJoiner $newJoiner): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => $newJoiner->load('candidate.jobVacancy'),
        ]);
    }
    ...
    public function convertToEmployee(
        NewJoiner $newJoiner,
        ConvertNewJoinerRequest $request
    ): JsonResponse {
        $this->authorize('proceedAsEmployee', NewJoiner::class);

        try {
            $employee = $this->conversionService->convertToEmployee(
                $newJoiner,
                $request->validated()
            );
        } catch (NewJoinerValidationException $e) {
